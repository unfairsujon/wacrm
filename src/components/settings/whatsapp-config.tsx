...
// Minimal typing for the bits of the Facebook JS SDK this file uses.
// The SDK itself has no first-party types; this only covers FB.init
// and FB.login as called here.
...
export function WhatsAppConfig() {
  ...
  // --- WhatsApp Embedded Signup (Facebook Business Manager one-click) ---
  // Meta's popup sends the WABA/phone number IDs via `postMessage`
  // (event 'WA_EMBEDDED_SIGNUP') independently of the FB.login()
  // callback, which only carries an OAuth `code`. We stash the
  // postMessage payload in a ref and combine it with the exchanged
  // token once both have arrived.
  const [embeddedSignupLoading, setEmbeddedSignupLoading] = useState(false);
  const [fbSdkReady, setFbSdkReady] = useState(false);
  const embeddedSignupDataRef = useRef<{
    phoneNumberId?: string;
    wabaId?: string;
  }>({});

  const metaAppId = process.env.NEXT_PUBLIC_META_APP_ID;
  const embeddedSignupConfigId = process.env.NEXT_PUBLIC_META_EMBEDDED_SIGNUP_CONFIG_ID;
  const embeddedSignupAvailable = Boolean(metaAppId && embeddedSignupConfigId);

  useEffect(() => {
    if (!embeddedSignupAvailable) return;

    function handleMessage(event: MessageEvent) {
      // Meta posts from https://www.facebook.com — ignore everything else.
      ...
      try {
        ...
        // Not a JSON embedded-signup message — ignore silently.
      }
    }
  ...
  return (
    <section className="animate-in fade-in-50 duration-200">
      ...
      <div className="space-y-6">
        ...
        {/* Embedded Signup — one-click connect via Facebook Business Manager */}
        {embeddedSignupAvailable && (
          <Card className="border-primary/30">
            <CardHeader>
              <CardTitle className="text-foreground">Connect with Facebook</CardTitle>
              <CardDescription className="text-muted-foreground">
                One click, using your Facebook Business Manager — no manual IDs or tokens needed.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Button
                onClick={handleEmbeddedSignup}
                disabled={!fbSdkReady || embeddedSignupLoading}
                className="w-full"
              >
